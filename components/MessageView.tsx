            ...
            <div style={{
              ...
              {canFork && (
                <button
                  ...
                  disabled={forking}
                  title={forking ? t("message.creatingSessionTitle") : t("message.newSessionTitle")}
                  style={{
                    display: "flex", alignItems: "center", gap: 4,
                    padding: "3px 8px", height: 22,
                    background: "none", border: "none",
                ...
                >
                  ...
                  {forking ? t("message.creatingSession") : t("message.newSession")}
                </button>
              )}
            </div>
          )}
        ...
        {isStreaming && (() => {
          ...
          return (
            <>
              ...
              {est > 0 && (
                <span style={{ display: "flex", alignItems: "center", gap: 4, color: "var(--text)" }} title={t("message.estimatedTokens")}>
...
function MermaidBlock({ code, isStreaming }: { code: string; isStreaming?: boolean }) {
  ...
  const previewButton = (
    <button
      ...
      title={isStreaming ? t("message.previewAfterStreaming") : (showPreview ? t("message.showMermaidSource") : t("message.previewMermaid"))}
      style={{
    ...
    >
      {showPreview ? t("common.source") : t("common.preview")}
    </button>
  );
  ...
  const body =
    failedKey === currentKey ? (
      <div className="mermaid-block mermaid-block-error">{t("message.invalidMermaid")}</div>
    ) : !svg || renderedKey !== currentKey ? (
      <div className="mermaid-block mermaid-block-loading" aria-label={t("message.renderingMermaid")} />
    ) : (
      <div
...
function PairedResult({ text, isEmpty, isError }: {
  ...
  return (
    ...
    >
      ...
      >
        {isEmpty ? t("message.noOutput") : text}
      </pre>
    </div>
  );
}
...
function formatUsage(usage: {
  ...
  const parts = [];
  if (usage.input) parts.push(t("message.usage.input", { value: usage.input.toLocaleString() }));
  if (usage.output) parts.push(t("message.usage.output", { value: usage.output.toLocaleString() }));
  if (usage.cacheRead) parts.push(t("message.usage.cacheRead", { value: usage.cacheRead.toLocaleString() }));
  if (usage.cacheWrite) parts.push(t("message.usage.cacheWrite", { value: usage.cacheWrite.toLocaleString() }));
