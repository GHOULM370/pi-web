        ...
        <div ref={topBarRef} style={{ display: "flex", alignItems: "center", flexShrink: 0, borderBottom: "1px solid var(--border)", height: 36, background: "var(--bg-panel)" }}>
          ...
          <button
            ...
            style={{
              ...
              fontSize: 11, fontWeight: 600, transition: "color 0.12s",
            }}
            onMouseEnter={(e) => { e.currentTarget.style.color = "var(--text)"; }}
            onMouseLeave={(e) => { e.currentTarget.style.color = "var(--text-muted)"; }}
          >
          ...
          {showChat && (sessionStats || contextUsage) && (() => {
            ...
            if (tokens) {
              tooltipParts.push(t("app.token.in", { value: tokens.input.toLocaleString() }));
              tooltipParts.push(t("app.token.out", { value: tokens.output.toLocaleString() }));
              tooltipParts.push(t("app.token.cacheRead", { value: tokens.cacheRead.toLocaleString() }));
              tooltipParts.push(t("app.token.cacheWrite", { value: tokens.cacheWrite.toLocaleString() }));
              if (c > 0) tooltipParts.push(t("app.token.cost", { value: `$${c.toFixed(4)}` }));
            }
            if (contextUsage?.contextWindow) {
              const pct = contextUsage.percent;
              tooltipParts.push(t("app.token.context", {
                percent: pct !== null ? pct.toFixed(1) + "%" : "unknown",
                tokens: contextUsage.contextWindow.toLocaleString(),
              }));
            }
              ...
              <div style={{ position: "absolute", top: 12, left: 12, display: "flex", alignItems: "flex-start", gap: 8, userSelect: "none", pointerEvents: "none" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "var(--text-muted)", lineHeight: 1.8 }}>
                    <span style={{ color: "var(--text-dim)", marginRight: 6 }}>1.</span>{t("app.getStarted.step1")}<br />
                    <span style={{ color: "var(--text-dim)", marginRight: 6 }}>2.</span>{t("app.getStarted.step2.prefix")} <strong style={{ color: "var(--text)" }}>{t("app.models")}</strong> {t("app.getStarted.step2.suffix")}
                  </div>
                </div>
              </div>
            )
